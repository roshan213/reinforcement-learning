...
OUTPUT_DIRECTORY = 'output_adjustedTransitionThirdQ/'

INPUT_PATH = OUTPUT_DIRECTORY
OUTPUT_PATH = OUTPUT_DIRECTORY+'images/'
REPORT_PATH = OUTPUT_DIRECTORY+'report/'
...
def plot_reward_and_delta_vs_steps(title, df, xlabel="Steps", ylabel="Reward"):
    ...
    lns1 = ax.plot(df.index.values, df['reward'], linewidth=1, label=ylabel, color="r")

    ex_ax = ax.twinx()
    lns2 = ex_ax.plot(df.index.values, df['delta'], linewidth=1, label='Delta', color="b")
